...
func TestShortLinkGenerator(t *testing.T) {
	...
	shortUrl_1 := GenerateShortLink(originalUrl_1, userId)

	originalUrl_2 := "github.com/papadonut9"
	shortUrl_2 := GenerateShortLink(originalUrl_2, userId)

	originalUrl_3 := "https://youtu.be/_sPXXL9YXok"
	shortUrl_3 := GenerateShortLink(originalUrl_3, userId)

	originalUrl_4 := "https://open.spotify.com/track/4QfVhHGCPVYLiQEnaeknJe"
	shortUrl_4 := GenerateShortLink(originalUrl_4, userId)

	originalUrl_5 := "https://open.spotify.com/track/2LMkwUfqC6S6s6qDVlEuzV"
	shortUrl_5 := GenerateShortLink(originalUrl_5, userId)
